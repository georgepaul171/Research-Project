    ...
    def fit(self, X: np.ndarray, y: np.ndarray, feature_names: Optional[list] = None, output_dir: Optional[str] = None) -> 'AdaptivePriorARD':
        ...
        for iteration in range(self.config.max_iter):
            ...
            if self.config.use_hmc:
                ...
                # Plot trace plots for all features
                if chains is not None and output_dir is not None and feature_names is not None:
...
if __name__ == "__main__":
    ...
    results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'resultshmc')
